  ...
  () => {
    ...
    module IdentityLoaderImpl = {
      ...
      let batchLoadFun = (strings) =>
        Js.Promise.resolve(Array.map((string) => Js.Result.Ok(string), strings));
    ...
    testPromise(
      ...
      () => {
        ...
        |> Js.Promise.then_((loadedVal) => Js.Promise.resolve(expect(loadedVal) |> toBe("1")))
      }
    );
    testPromise(
      ...
      () => {
        ...
        |> Js.Promise.then_(
             (loadedVal) => Js.Promise.resolve(expect(loadedVal) |> toEqual([|"1", "2"|]))
           )
      }
    );
    testPromise(
      ...
      () => {
        ...
        |> Js.Promise.then_(
             (loadedValues) => Js.Promise.resolve(expect(loadedValues) |> toEqual([|"1", "2"|]))
           )
      }
    );
    testPromise(
  ...
  () => {
    ...
    testPromise(
      ...
      () => {
        ...
        Js.Promise.all([|load1, load2|])
        |> Js.Promise.then_(
             (loadedValues) => Js.Promise.resolve(expect(loadedValues) |> toEqual([|"A", "2"|]))
           )
      }
    )
  }
  ...
  () => {
    ...
    testPromise(
      ...
      () => {
        ...
        Js.Promise.all([|load1b, load2b|])
        |> Js.Promise.then_(
             (loadedValues) => Js.Promise.resolve(expect(loadedValues) |> toEqual([|"X", "Y"|]))
           )
      }
    )
  }
